...
def run():
    ...
    hbar = tk.Scrollbar(
        main_frame, orient = tk.HORIZONTAL, command = UMLBox.test_canvas.xview)
    hbar.pack(fill = tk.X, side = tk.BOTTOM)

    vbar = tk.Scrollbar(
        main_frame, orient = tk.VERTICAL, command = UMLBox.test_canvas.yview)
    ...
    UMLBox.test_canvas.bind(
        '<Configure>', lambda e: UMLBox.test_canvas.configure(
            scrollregion = UMLBox.test_canvas.bbox("all")))
    
    # Element Placement
    main_frame.grid(
        row = 0, column = 1, sticky = "nsew", rowspan = 2)
    main_frame.columnconfigure(1, weight = 1)
    main_frame.rowconfigure(1, weight = 1)
    
    side_frame.grid(
        row = 0, column = 2, sticky = "nse", rowspan = 2)
...
def build_menu(window : tk.Tk) -> tk.Menu:
    ...
    menu_file.add_command(label = "Save", command = gw.save_window)
    menu_file.add_command(label = "Load", command = gw.load_window)
    menu_file.add_command(label = "Export", command = gw.export_window)
    menu_file.add_separator()
    menu_file.add_command(label = "Exit", command = exit)
    menubar.add_cascade(label = "File", menu = menu_file)
    
    menu_edit = tk.Menu(menubar, tearoff = 0)
    menu_edit.add_command(label = "Undo", command = gf.b_undo)
    menu_edit.add_command(label = "Redo", command = gf.b_redo)
