...
def test_two_uml_class () :
    ...
    assert class_interface.__name__ == "class1"
    assert class_interface.__name__ == "class2"
...
def test_delete_ne_uml_class () :
    uml = UMLClass ()
    uml.class_dict.delete_class ("class1")
    out = io.StringIO ()
    sys.stdout = out
    snake_uml.main (sys.argv)
    sys.stdout = sys.__stdout__
